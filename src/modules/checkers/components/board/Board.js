  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      arr.push({
        ...
        j,
        id: acc,
        background: (i + j) % 2 === 0 ? "#000000" : "#ffffff",
      });
...
const Board = ({ rows = 8, cols = 8, checked }) => {
  ...
  const changeMapple = (id) => {
    setVisibleMapples({
      ...visibleMapples,
      [id]: visibleMapples[id] ? false : true,
    });
  };

  const checkMoveCorrect = (currentRow, row, currentCol, col) => {
  ...
  const moveMapple = (id) => {
    const foundBoardItemCur = board.find((boardItem) => boardItem.id === id);
    const foundBoardItemPrev = board.find(
      (boardItem) => boardItem.id === selectedMapple
    );
    if (
      checkMoveCorrect(
        foundBoardItemCur.i,
        foundBoardItemPrev.i,
        foundBoardItemCur.j,
        foundBoardItemPrev.j
      )
    ) {
      setVisibleMapples({
        ...visibleMapples,
